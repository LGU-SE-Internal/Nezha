            ...
            try:
                ...
                if len(parent_pod_span) > 0:
                    for parent_span_index in range(len(parent_pod_span["PodName"])):
                        parent_pod_name = parent_pod_span["PodName"].iloc[
                            parent_span_index
                        ]
                        parent_end_time = int(
...
def generate_alarm(metric_list, ns, std_num=6):
    ...
    for pod_metric in metric_list:
        ...
        for i in range(len(pod_metric["metrics"])):
            alarm_flag = determine_alarm(
                pod=pod_metric["pod"],
                metric_type=pod_metric["metrics"][i]["metric_type"],
                metric_value=pod_metric["metrics"][i]["metric_value"],
                std_num=std_num,
                ns=ns,
            )
            if alarm_flag:
        ...
        if "metric" in path:
            ...
            for index in range(len(metrics["Time"])):
                ...
                if re.search(time, metrics["Time"][index]):
                    target_metric = {"pod": metrics["PodName"][index], "metrics": []}
                    for metric in metric_list:
                        target_metric["metrics"].append(
                            {
                                "metric_type": metric,
                                "metric_value": metrics[metric][index],
                            }
                        )
